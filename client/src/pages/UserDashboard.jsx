...
const UserDashboard = () => {
  ...
  const handlePasswordChange = async () => {
    ...
    if (!passwordRegex.test(password)) return alert('Password criteria not met');
    try {
  ...
  return (
    <div className="container">
      ...
      <div className="card mb-4 flex gap-2 items-center flex-wrap">
        <input value={search} onChange={e => setSearch(e.target.value)} placeholder="Search Store Name or Address..." style={{ marginTop: 0, flex: 1 }} />

        <div className="flex gap-2 items-center">
          <span className="text-sm">Sort:</span>
          <select value={sort} onChange={e => setSort(e.target.value)} style={{ width: 'auto', marginTop: 0 }}>
          ...
          <div key={store.id} className="card" style={{ marginBottom: 0 }}>
            ...
            <div className="flex justify-between items-center mt-4" style={{ borderTop: '1px solid var(--border-color)', paddingTop: '1rem' }}>
              <div>
                ...
                <div style={{ fontWeight: 'bold', fontSize: '1.25rem', color: 'var(--primary-color)' }}>⭐ {store.overallRating}</div>
              </div>
